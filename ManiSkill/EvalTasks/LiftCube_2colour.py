...
def eval_once(target_colour):
    ...
    # rip off all wrappers and get the core env
    core = env.unwrapped
    
    # record the blue cube's initial Z-height
    init_blue_cube_z = raw_observation["extra"]["blue_cube_pose"][0][2]
    
    # record the red cube's initial Z-height
    init_red_cube_z = core.cube.pose.p[0, 2].item()
    ...
    while not done:
        ...
        # check the blue cube's current Z
        curr_blue_cube_z = raw_observation["extra"]["blue_cube_pose"][0][2]
        
        # check the red cube's current Z
        curr_red_cube_z = core.cube.pose.p[0, 2].item()
        
        # Get target cube's pos
        if target_colour == "red":
            curr_cube_z = curr_red_cube_z
            init_cube_z = init_red_cube_z
        elif target_colour == "blue":
            curr_cube_z = curr_blue_cube_z
            init_cube_z = init_blue_cube_z
...
if __name__ == "__main__":
    ...
    # Load Policy
    pretrained_policy_path = "Calvert0921/smolvla_franka_liftcubesdouble_2000_mixed_10k"
    dataset = LeRobotDataset("Calvert0921/SmolVLA_LiftCubesDouble_Franka_2000", download_videos=False)
